...
0.090772 (up to Minion 9.07/100+C<minor>)
...
=head2 Pg => <connection url string> | <Mojo::Pg object> | <coderef>

If config parameter B<Pg> is a B<coderef> then returned value
of C<< $conf->{Pg} = $conf->{Pg}->($app); >> will reassigned to B<Pg> config.
...
B<ALL> exists Minion processes stoped and then running.
